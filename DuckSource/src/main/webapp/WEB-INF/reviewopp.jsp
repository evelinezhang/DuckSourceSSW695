    ...
    <div class="art-layout-cell layout-item-3" style="width: 100%" >                         
						...
						<c:choose>
							...
							<tr>
								...
								<td style="border-width: 1px; text-align: left; border-right:none; vertical-align: middle;">
									${s.status} 
								</td>
								<td style="border-width: 1px; text-align: right; border-left:none; vertical-align: middle;"> 
								...
								<td style="border-width: 1px; text-align: center; vertical-align: middle;">
									...
									<c:forEach var="r" items="${reviewIssues}">
										<c:if test="${r.opportunitySubmitted.id == s.id}">
    										<c:set var="reviewed" value="true" />
  										</c:if>
									</c:forEach>								
									<c:choose>
						    			<c:when test="${reviewed eq true}">
						    				 <a href="reviewoppissues?userId=${userId}&oppId=${opportunity.id}&subId=${s.id}">View Review Issues</a>
						    			</c:when>    
						    			<c:otherwise>
						    				<a href="addissue?userId=${userId}&oppId=${opportunity.id}&subId=${s.id}&loc=0">Add Review Issue</a>
										</c:otherwise>
									</c:choose>
								</td>
							</tr>
